...
- compact **190 x 99 x 31.5 mm** enclosure sized around one full-width 10x2 reed cartridge
- integrated shallow **humidity bay** below the cartridge, holding **2 x Boveda Size 8** packs under a simple vented drop-in cover
- one removable double-sided 10-reed-per-face tray / cartridge
...
The cartridge sits on a raised support frame above the humidity bay, leaving a
vented service cavity below for the two Boveda packs. The side-latch hardware stays
in its own narrow service strips beside the cartridge.
...
Use `humidity_cassette` or `humidity_bay_open` to inspect the humidity bay with its cover lifted.
Use `humidity_bay_closed` to preview the lid seated on its ledge.
`case_exploded` now shows the base, the vented humidity cover over two packs, the full reed
cartridge above it, and the lid.
...
make export-humidity-cover    # vented humidity cover
make export-tray-face-a       # only face A
make export-tray-face-b       # only face B
make export-tray              # both support-free shell halves
```
...
The current cartridge has **no middle/core layer**. It is built from two
slightly thicker mirrored shell halves that join directly back-to-back.
`make export-tray` exports those two support-free halves; `make export-tray-full`
exports the fused cartridge as one STL. The heel-side 65% uses five smooth
longitudinal breathing runners per reed lane, while the tip-side 35% is fully
open air.
...
## Side latch metal hardware

The current case uses two slim side buttons with bowed 301 stainless leaf
springs hidden behind removable stainless cover plates. See
`hardware/SIDE_LATCH_HARDWARE.md`. Export the cutting templates with:
