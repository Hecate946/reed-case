...
sheet_part = is_undef(sheet_part) ? "front_latch_cover" : sheet_part;
mesh_profile = is_undef(mesh_profile) ? "fine" : mesh_profile;

include <config.scad>
include <assembly.scad>

if (sheet_part == "front_latch_cover" || sheet_part == "side_latch_cover")
    side_latch_cover_plate_2d();
else if (sheet_part == "front_leaf_spring" || sheet_part == "side_leaf_spring")
